...
# Laster pakker og sletter minnet.
library(tidyverse)
rm(list = ls())

# Definerer intervall og lagrer i dataframe.
t <- seq(0, 100, by = 1) 
df <- data.frame(t)

# Definerer variabler i situasjon 1 og 2. 
y0 <- 600
alpha <- 0.2
beta <- 0.6
gamma <- 0.2
...
# Beregner g_y i situasjon 1 og 2.
...
print(gy0) # Vekstraten i situasjon 1
print(gy1) # vekstraten i situasjon 2

# Definerer funksjon for vekst i t antall ??r.
yt_0 <- function(t) (y0*exp(gy0*t))
yt_1 <- function(t) (y0*exp(gy1*t))

# Definerer akser til figuren.
...
# Lager figuren i situasjon 1 og 2 ilag. 
figur_3 <- axes_1 +
  stat_function(df,
                fun=yt_0,
                mapping = aes() # vekst i situasjon 0
